...
const CoursePage = ({ onLogout }: { onLogout: () => void }) => {
  ...
  const handleHomeworkDone = (idx: number) => {
    updateElement(idx, { step: "completed" });
    if (idx >= unlockedUpTo) {
      setUnlockedUpTo(idx + 1);
      if (idx + 1 < courseElements.length) {
        updateElement(idx + 1, { expanded: true });
      }
    }
  };
    ...
    <div className="min-h-screen bg-background">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="hidden md:flex items-center gap-3">
              <span className="text-sm text-muted-foreground font-medium">
                {Math.min(unlockedUpTo, courseElements.length)}/{courseElements.length} пройдено
              </span>
            ...
            <div className="flex md:hidden items-center gap-2">
              <span className="text-xs text-muted-foreground font-medium">
                {Math.min(unlockedUpTo, courseElements.length)}/{courseElements.length}
              </span>
      ...
      <div className="bg-muted h-1">
        <div
          ...
          style={{ width: `${(unlockedUpTo / courseElements.length) * 100}%` }}
        />
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-3 pb-20">
          ...
          return (
            ...
            >
              ...
              >
                ...
                {!isLocked && (
                  <i
                    className={`fas fa-chevron-${state.expanded ? "up" : "down"} text-muted-foreground text-xs`}
                  />
                )}

                {isLocked && (
                  <i className="fas fa-lock text-muted-foreground text-xs" />
                )}
              </button>
                ...
                <div className="px-4 pb-4 space-y-5">
                  ...
                  {state.step === "quiz" && (
                    <div className="bg-background rounded-lg border border-border p-4">
                      ...
                      <QuizSection
                        questions={el.quiz}
                        onComplete={(score) => handleQuizComplete(idx, score)}
                      />
                    </div>
                  )}
        ...
        {unlockedUpTo >= courseElements.length && (
